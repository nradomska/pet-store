...
test.describe('add-pet', () => {
  test('should add pet', async ({ page }) => {
    ...
    await page.locator('.pet-form__name').fill('Nie usuwać');
    await page.locator('.pet-form__category').fill('kategoria');
    await page.locator('#mat-select-value-2').click();
    await page.locator('#mat-option-7').click();
    await page.locator('.pet-form__photo').click();
    await page.locator('.pet-form__photo').fill('https://t4.ftcdn.net/jpg/00/27/27/71/360_F_27277141_4uhRVl4hJgtacBgDPla0sklkxaFWFFPA.jpg');
    await page.locator('.pet-form__tag').click();
...
test.describe('edit-pet', () => {
  test('should edit pet', async ({ page }) => {
    ...
    await page.getByPlaceholder('Filter by name').fill('nie usuwać');
    await page.locator('.pets-view__action-button--edit').first().click();
    await page.locator('.pet-form__name').fill('Nie usuwać po edycji');
    await page.locator('.pet-form__submit button').click();

    await page.waitForLoadState();
    await expect(page.locator('mat-snack-bar-container')).toBeVisible();
    await expect(page.getByRole('cell', { name: 'Nie usuwać po edycji' }).first()).toBeVisible();
  });
});
